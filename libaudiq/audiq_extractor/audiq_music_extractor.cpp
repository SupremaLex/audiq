...
void AudiqMusicExtractor::reset() {}
...
#else
    E_WARNING("AudiqMusicExtractor: Gaia library is missing. Skipping configuration of SVM models.");
#endif
  }
}


void AudiqMusicExtractor::setExtractorDefaultOptions() {
  // general
...
void AudiqMusicExtractor::compute() {
  ...
  if (requireMbid
      ...
      throw EssentiaException("AudiqMusicExtractor: Error processing ", audioFilename, " file: cannot find musicbrainz recording id");
  }
  ...
  if (options.value<Real>("highlevel.compute")) {
    ...
    _svms->compute();
#else
    E_WARNING("AudiqMusicExtractor: Gaia library is missing. Skipping computation of SVM models.");
#endif
  }
  E_INFO("All done");
  resultsStats = stats;
...
/*void AudiqMusicExtractor::readMetadata(const string& audioFilename, Pool& results) {
...
void AudiqMusicExtractor::computeAudioMetadata(const string& audioFilename, Pool& results) {
  streaming::AlgorithmFactory& factory = streaming::AlgorithmFactory::instance();
  ...
  if (!analysis_length) {
    ostringstream msg;
    msg << "AudiqMusicExtractor: empty input signal (analysis startTime: " << startTime
